...
class User extends \Core\Model
{
    ...
    public function validate()
    {
        // Name
        if ($this->name == '') {
            $this->errors[] = 'Proszę podać imię';
        }

        // email address
        if (filter_var($this->email, FILTER_VALIDATE_EMAIL) === false) {
            $this->errors[] = 'Podano niepoprawny adres email';
        }
        if (static::emailExists($this->email, $this->id ?? null)) {
            $this->errors[] = 'Adres email został zapisany';
        }

        // Password
        if ($this->password != $this->password_confirmation) {
            $this->errors[] = 'Hało musi uzyskać potwierdzenie';
        }

        if (strlen($this->password) < 6) {
            $this->errors[] = 'Hasło musi posiadać przynamniej 6 znaków';
        }
        ...
        if (preg_match('/.*\d+.*/i', $this->password) == 0) {
            $this->errors[] = 'Hasło musi posiadać przynajmniej jdną cyfrę';
        }
    }

    public static function emailExists($email, $ingnore_id = null)
    {
